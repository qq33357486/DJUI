...
- [ ] `cd editor/frontend; npx tsc --noEmit` 通过。
- [ ] `cd editor/frontend; npm run build` 通过。
...
- [ ] StarEngine 客户端可调用 `DjuiWindowManagerV6.Initialize()`。
- [ ] StarEngine 客户端可调用 `DjuiWindowManagerV6.OpenWindow("页面ID")`。
...
- [ ] 修改 Runtime 时同步更新 `docs/runtime.md` 与 `runtime/AGENTS.md`。
- [ ] 修改安全模型时同步更新 `docs/security.md`。
...
- [ ] 修改素材分类时同步：
  ...
  - `editor/frontend/src/fs/fsAccess.ts` 的 `IMAGE_EXTS`（仅新增图片格式时）

## 安全和仓库卫生

- [ ] 没有真实本机路径、密钥、Token、私有服务器地址。
- [ ] 没有临时截图、调试日志、旧需求草稿进入提交。
- [ ] Docker 镜像不包含任何服务端逻辑（Nginx 纯静态托管）。

...
export default function LeftPanel({ pages, onNewPage, onSwitchPage, onDeletePage }: LeftPanelProps) {
  ...
  return (
    <div className="djui-left-panel" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      {/* 拖拽指示线颜色：放入子节点(inside=drag-over 橙) vs 交换位置(gap-top/bottom 蓝) */}
      <style>{`
        .djui-left-panel .ant-tree-treenode.drag-over .ant-tree-drop-indicator { background-color: #ff8c42 !important; }
        .djui-left-panel .ant-tree-treenode.drag-over-gap-top .ant-tree-drop-indicator,
        .djui-left-panel .ant-tree-treenode.drag-over-gap-bottom .ant-tree-drop-indicator { background-color: #5ab9ff !important; }
      `}</style>
      {/* === 标题栏 + 新建按钮 === */}
      <div style={{
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
